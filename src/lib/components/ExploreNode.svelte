<script lang="ts">
	...
	let {
		...
		scope,
		filter,
	}: { k: string | number; v: unknown; scope: (string | number)[]; filter: string } = $props();
	...
	const matches = (key: string | number) =>
		!filter || minimatch([...myScope, key].join("/"), filter, { partial: true });
	...
	const childEntries = $derived.by(() => {
		if (Array.isArray(data)) {
			return data
				.map((value, idx) => [idx + 1, value] as [number, unknown])
...
<li class="flex items-start gap-1 py-px">
	<Button
		variant="ghost"
		size="icon-sm"
		...
		{#if childEntries.length > 0}
			<ul>
				{#each childEntries as [key, value] (`${minimatchAddress}/${key}`)}
					<ExploreNode k={key} v={value} scope={myScope} {filter} />
				{/each}
			</ul>
		{/if}
	</div>
